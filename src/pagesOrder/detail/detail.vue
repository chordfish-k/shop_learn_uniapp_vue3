...
import { getMemberOrderByIdAPI, getMemberOrderConsignmentByIdAPI, deleteMemberOrderAPI, putMemberOrderReceiptByIdAPI, getMemberOrderLogisticsByIdAPI, getMemberOrderCancelByIdAPI } from '@/services/order'
...
// 当页面渲染完毕，绑定动画效果
onReady(() => {
  // 动画效果(使用wx的api，不支持ts类型，使用any类型屏蔽类型检查)
...
<template>
  ...
  <scroll-view scroll-y
    ...
    <template v-if="order">
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item in logisticList"
              :key="item.id"
              class="item">
          <view class="message">
            {{ item.text }}
      ...
      <view class="detail">
        ...
        <view class="row">
          <view class="item">
            订单编号: {{ query.id }} <text class="copy"
                  @tap="onCopy(query.id)">复制</text>
          </view>
      ...
      <view class="toolbar-height"
            :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }"></view>
      <view class="toolbar"
    ...
    <view class="popup-root">
      ...
      <view class="description">
        ...
        <view class="cell"
          ...
          <text class="icon"
                :class="{ checked: item === reason }"></text>
        </view>
      </view>
      <view class="footer">
        <view class="button"
              @tap="popup?.close?.()">取消</view>
        <view class="button primary"
              @tap="onOrderCancel">确认</view>
      </view>
    </view>
  </uni-popup>
</template>
